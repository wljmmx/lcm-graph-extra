...
export function registerOperationalTools(api: any): void {
  ...
  api.registerTool({
    ...
    description: "Extract experience summaries from the Neo4j knowledge graph. " +
  ...
  api.registerTool({
    name: "lcmg_backup",
    description: "Backup ALL memory data: Neo4j graph + lossless-claw messages + memory markdown files",
    parameters: Type.Object({
  ...
  api.registerTool({
    name: "lcmg_restore",
    description: "Restore memory data from a backup JSON into Neo4j + lossless-claw DB + memory files",
    parameters: Type.Object({
  ...
  api.registerTool({
    name: "lcmg_import",
    description: "Import historical data from lossless-claw DB or memory files into Neo4j knowledge graph." +
  ...
  api.registerTool({
    name: "lcmg_diagnose",
    description: "Comprehensive health report: lossless-claw DB, qmd, Neo4j, circuit breakers",
    parameters: Type.Object({}),
    async execute() {
      const L: string[] = [];
...
// ===================================================================
  ...
  api.registerTool({
    name: "lcmg_search",
    description: "Cross-engine search: queries lossless-claw FTS5 + qmd BM25/vector + Neo4j knowledge graph. " +
  ...
  api.registerTool({
    name: "lcmg_pin",
    description: "Mark a Neo4j node as pinned (excluded from TTL-based memory decay). " +
  ...
  api.registerTool({
    name: "lcmg_sync",
    description: "Repair data consistency across lossless-claw DB / Neo4j / memory files. " +
