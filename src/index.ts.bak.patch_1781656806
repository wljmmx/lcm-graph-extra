...
/** Check if a category tool is self-registered (independent of Tool Search mode). */
        ...
        try {
          ...
          if (availableTools.length > 0) {
            logger?.debug?.(
              ...
              ", modelCanCall->graph=" + hasGraphTool + ",exp=" + hasExperienceTool +
              ", selfRegistered->graph=" + selfHasGraph + ",exp=" + selfHasExp
            );
          }
          else {
            logger?.debug?.(
              "[lcm-graph-extra] Tool Search mode active (availableTools empty)," +
              " selfRegistered->graph=" + selfHasGraph + ",exp=" + selfHasExp
            );
          ...
          if (_l2m.includes("circuit breaker")) {
            ...
            logger?.warn?.("L2 qmd: error - " + _l2m);
              ...
              (async () => {
                ...
                try {
                  if (!selfHasGraph) {
                    logger?.debug?.("[lcm-graph-extra] L3 graph search skipped (category not self-registered)");
                    return { results: [], ms: 0 };
                  }
              ...
              (async () => {
                ...
                try {
                  if (!selfHasExp) {
                    logger?.debug?.("[lcm-graph-extra] L4 experience search skipped (category not self-registered)");
                    return { results: [], ms: 0 };
                  }
