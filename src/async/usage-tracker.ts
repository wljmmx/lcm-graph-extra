...
class TokenCounter {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      this.ready = true;
      this.logger.log('[usage-tracker] Token counter ready');
    } catch (err) {
  ...
  async init(): Promise<void> {
    try {
      ...
      if (BetterSqlite3?.default) {
        ...
        this.initSchema();
        this.ready = true;
        this.logger.log('[usage-tracker] DB ready (better-sqlite3)');
      } else {
        // Fallback: JSON file storage
        this.logger.log('[usage-tracker] DB fallback: JSON file storage');
