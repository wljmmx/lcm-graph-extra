 ...
 *
 * 将 dashboard 前端的请求代理到 graph-memory-pro 插件自身的 HTTP 服务器，
 * 用于获取 graph-memory-pro 的状态、统计、图谱数据等信息。
 *
 * 代理路径映射：
 ...
 *
 * 配置：
 *   GM_PRO_HTTP_URL —— graph-memory-pro HTTP 服务器地址（默认 http://127.0.0.1:7424）
 *   GM_PRO_HTTP_TIMEOUT —— 代理超时（ms），默认 10s
 *
 * 安全：
 *   - 仅允许 GET 请求（只读），拒绝 POST/PATCH/PUT/DELETE 写操作
 *   - 出站请求携带 DASHBOARD_AUTH 凭据
 *   - 路径白名单校验，防止 SSRF 遍历
 */
...
export async function registerGmProRoutes(app: FastifyInstance): Promise<void> {
   ...
   *
   * 将请求路径中 /api/gm-pro/proxy/ 之后的部分拼接到 graph-memory-pro 的 HTTP URL。
   *
   * 示例：
   *   请求: GET /api/gm-pro/proxy/status
   *   代理: GET http://127.0.0.1:7424/api/status
   */
